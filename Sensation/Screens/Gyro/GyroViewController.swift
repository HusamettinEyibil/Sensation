...
class GyroViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        navigationItem.title = "Gyro"
        
        tableView.delegate = self
        tableView.dataSource = self
        tableView.register(UINib(nibName: GyroCell.identifier, bundle: nil), forCellReuseIdentifier: GyroCell.identifier)
    ...
    func updateValues() {
        let newModel = GyroModel(creationTime: getCurrentTime(), values: gyroManager.getValues())
...
extension GyroViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: GyroCell.identifier, for: indexPath) as! GyroCell
        if indexPath.row == 0 {
            cell.dateLabel.text = ""
            cell.xLabel.text = "X"
            cell.yLabel.text = "Y"
